...
export class TextLexIndice<T> implements ISpreadIndice<T, string>{
        ...
        this.keys.forEach((key) => {
            ...
            if (size + value.length <= chunkSize) {
                ...
                while (value.length) {
                    ...
                    result.push(TextLexIndice.deserialize<T>(
                        map,
                        options
                    ));
                    size = 0;
    ...
    public async findAll(indices: ISpreadIndice<T, string>[], value?: string | string[], { operator = '$eq', sort = 1 }: Partial<IFindOptions> = {}): Promise<T[]> {
        let tokens: string[] = []
        if (value !== undefined) {
            tokens = Array.isArray(value) ? value.flatMap(v => this.tokenizr(v)) : this.tokenizr(value);
        }
    ...
    public cursorAll(indices: ISpreadIndice<T, string>[], value?: string | string[], { operator = '$eq', sort = 1, chunkSize = 20 }: Partial<IFindOptions> = {}): AsyncIterable<T[]> {
        let tokens: string[] = []
        if (value !== undefined) {
            tokens = Array.isArray(value) ? value.flatMap(v => this.tokenizr(v)) : this.tokenizr(value);
        }
